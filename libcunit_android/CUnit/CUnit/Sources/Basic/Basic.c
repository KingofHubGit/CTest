...
static CU_ErrorCode basic_initialize(void)
{
  ...
  if (CU_BRM_SILENT != f_run_mode)
    fprintf(stdout, "\n\n     %s" CU_VERSION
                      "\n     %s\n\n",
                    _("Datalogic BSP-API Test Report"),
                    _("CUnit - A unit testing framework for C - Version (http://cunit.sourceforge.net/)"));
                                                ...
                                                const CU_pFailureRecord pFailureList)
{
  ...
  if (NULL == pFailure) {
    if (CU_BRM_VERBOSE == f_run_mode) {
      fprintf(stdout, "\033[0;32m PASS\033[0m\n");
    }
  }
  else {
    switch (f_run_mode) {
      case CU_BRM_VERBOSE:
        fprintf(stdout, "\033[0;31m FAILED\033[0m\n");
        break;
      case CU_BRM_NORMAL:
        assert(NULL != pSuite->pName);
        assert(NULL != pTest->pName);
